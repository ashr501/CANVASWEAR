...
function getEnv(): Env {
  ...
  return {
    ...
    BRAND4_STORE_DOMAIN: p.BRAND4_STORE_DOMAIN ?? '',
    BRAND4_STOREFRONT_API_TOKEN: p.BRAND4_STOREFRONT_API_TOKEN ?? '',
    R2_ACCOUNT_ID: p.R2_ACCOUNT_ID ?? '',
    R2_ACCESS_KEY_ID: p.R2_ACCESS_KEY_ID ?? '',
    R2_SECRET_ACCESS_KEY: p.R2_SECRET_ACCESS_KEY ?? '',
    R2_BUCKET: p.R2_BUCKET ?? '',
    R2_PUBLIC_BASE_URL: p.R2_PUBLIC_BASE_URL ?? '',
  };
}
