...
商品に **`custom-print` タグ**を付けると、商品ページに以下が表示されます。

1. **デザインデータのアップロード欄**（PNG / JPEG / WebP / SVG / PDF・20MBまで）
2. **ご指定・ご要望の入力欄**（文字・配置・色味など）

アップロードされたファイルはCloudflare R2に保存され、その公開URLが
注文明細の line item property として記録されます。カート画面とShopifyの
注文画面の両方に表示されるので、注文を見ればそのまま入稿データを開けます。
...
## Cloudflare R2 の設定（入稿ファイルの保管先）

R2は**月10GBの保存と、月100万回の書き込みまで無料**です。
R2からの読み出しに転送料（エグレス料金）はかかりません。

### 1. バケットを作る

Cloudflareダッシュボード → **R2** → **Create bucket**
- バケット名: `canvaswear-uploads`（例）
- ロケーション: **Asia-Pacific (APAC)**

### 2. 公開アクセスを有効にする

作ったバケット → **Settings** → **Public access** → **R2.dev subdomain** を Allow

`https://pub-xxxxxxxx.r2.dev` のようなURLが発行されます。これが
`R2_PUBLIC_BASE_URL` です。独自ドメインを割り当てることもできます。

### 3. APIトークンを発行する

R2のトップ → **Manage R2 API Tokens** → **Create API token**
- 権限: **Object Read & Write**
- 対象: 作成したバケットのみ

発行される **Access Key ID** と **Secret Access Key** を控えます
（Secretは一度しか表示されません）。
...
| 変数 | 値 |
|---|---|
| `R2_ACCOUNT_ID` | CloudflareのアカウントID（R2の画面右側に表示） |
| `R2_ACCESS_KEY_ID` | 手順3のAccess Key ID |
| `R2_SECRET_ACCESS_KEY` | 手順3のSecret Access Key |
| `R2_BUCKET` | `canvaswear-uploads` |
| `R2_PUBLIC_BASE_URL` | 手順2のURL（末尾のスラッシュなし） |

Oxygenの場合はストアフロント設定の環境変数、Vercelの場合はプロジェクトの
環境変数に登録します。**未設定でもサイトは動きます**（入稿欄がエラーを返すだけ）。

### 運用上の注意

- アップロード欄は**ログイン不要で誰でも投稿できます**。形式（5種類）と
  サイズ（20MB）の制限はかけていますが、いたずら投稿は防げません。
  公開前にCloudflareのWAFでレート制限をかけることをおすすめします。
- 保存したファイルは**自動削除されません**。注文が完了して印刷が済んだ
  データは、R2のライフサイクルルール（例: 90日で削除）で整理してください。
- 公開URLを知っていれば誰でも閲覧できます。推測されにくいUUIDを使って
  いますが、機密性の高いデータの入稿には向きません。
